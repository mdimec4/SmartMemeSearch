...
namespace SmartMemeSearch.Wpf.Services
{
    public class AutoSyncService
    {
            ...
            Action<double> onProgress)
        {
            ...
            // Build quick lookup of path → lastModified only for this folder
            var dbItems = new Dictionary<string, long>();

            foreach (var item in _db.GetAllEmbeddings())
            {
                if (item.FilePath.StartsWith(folder + Path.DirectorySeparatorChar,
            ...
            // Remove files that belong to THIS folder only
            _db.RemoveMissingFiles(images, folder);
